...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="tree-card">
      <div class="tree-formula-box">
        ...
        {'Com repetição: N = n^k = ' + str(n) + '^' + str(k) + ' = ' + str(total)
         if rep else
         'Sem repetição: N = P(n,k) = ' + str(n) + '!/' + str(n-k) + '! = ' + str(total)}<br><br>

        <b>2. Profundidade mínima da árvore (cota de Shannon)</b><br>
        Cada feedback elimina parte das possibilidades. No melhor caso:<br>
        &nbsp;&nbsp;&nbsp;t<sub>min</sub> = ⌈log<sub>2</sub>(N)⌉ = ⌈log<sub>2</sub>({total})⌉ = <b>{math.ceil(math.log2(total)) if total > 1 else 1} tentativas</b><br><br>

        <b>3. Fator de ramificação</b><br>
        O número de feedbacks distintos por tentativa determina o fator de ramificação.
        Para código de {k} posições, o número máximo de feedbacks distintos é:<br>
        &nbsp;&nbsp;&nbsp;F = (k+1)(k+2)/2 − 1 = <b>{((k+1)*(k+2)//2) - 1} feedbacks</b><br><br>

        <b>4. Estratégia ótima</b><br>
        A tentativa mais informativa é aquela que divide o espaço de possibilidades
        de forma mais equilibrada — minimizando o pior caso (estratégia minimax).
      </div>
    </div>
    """, unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    # ── Tentativa ativa ───────────────────────────────────────────────────────
        ...
        {"| ⚪ <b>Pino branco</b>: cor certa MAS posição errada" if st.session_state.use_white_pegs else "| modo sem pinos brancos ativo"}
        </div>
        """, unsafe_allow_html=True)
