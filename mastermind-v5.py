...
.bull-chip{background:#dcfce7;border:1px solid #86efac;border-radius:8px;padding:.2rem .55rem;font-size:.8rem;font-weight:700;color:#166534;display:flex;align-items:center;gap:3px;white-space:nowrap;font-family:'Nunito',sans-serif;}
.cow-chip{background:#fef3c7;border:1px solid #fcd34d;border-radius:8px;padding:.2rem .55rem;font-size:.8rem;font-weight:700;color:#92400e;display:flex;align-items:center;gap:3px;white-space:nowrap;font-family:'Nunito',sans-serif;}
...
if st.session_state.screen == "intro":
    ...
    <div class="menu-grid">
      ...
      <div class="menu-card inc">
        ...
        <div class="menu-title">Vacas e Bois</div>
        <div class="menu-desc">Versão inclusiva: 2 símbolos, visual ampliado e linguagem simples.</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c3:
        if st.button("🐄 VACAS E BOIS", use_container_width=True):
    ...
    <div class="rules-card">
      ...
      <div class="rule-item">
        <span class="rule-icon">🐄🐂</span>
        <div>
          <span class="rule-title">Vacas e Bois — modo inclusivo</span>
          <span class="rule-desc">Apenas 2 símbolos visuais (Vaca e Boi), 4 posições fixas, botões grandes e feedback em português simples. Indicado para alunos com transtornos cognitivos ou como introdução ao raciocínio lógico.</span>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="inc-title">
      <h1>🐄 Vacas e Bois 🐂</h1>
      <p>Descubra a sequência secreta de animais!</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("📖 Ver as regras do jogo", expanded=False):
        ...
        <div class="inc-rules-card">
          ...
          <div class="inc-rule-row">
            ...
            <div>
              <span class="inc-rule-b">Boi = animal certo, lugar certo!</span>
              <span class="inc-rule-s">O animal está na posição exata do código secreto.</span>
            </div>
          </div>
          <div class="inc-rule-row">
            <span class="inc-rule-emoji">🐄</span>
            <div>
              <span class="inc-rule-b">Vaca = animal certo, mas lugar errado!</span>
              <span class="inc-rule-s">O animal existe no código, mas está em outra posição.</span>
            </div>
          </div>
          <div class="inc-rule-row">
            <span class="inc-rule-emoji">🎯</span>
            <div>
              <span class="inc-rule-b">Objetivo: 4 Bois!</span>
              <span class="inc-rule-s">Quando você acertar os 4 animais nas posições certas, vence!</span>
            </div>
          </div>
          <div class="inc-rule-row">
            <span class="inc-rule-emoji">⏳</span>
    ...
    if st.session_state.inc_history:
        ...
        for i, (guess, bulls, cows) in enumerate(st.session_state.inc_history, 1):
            ...
            bull_s = "bois" if bulls != 1 else "boi"
            cow_s  = "vacas" if cows != 1 else "vaca"
            st.markdown(f"""
            <div class="inc-row">
              <span class="inc-row-num">#{i}</span>
              <div class="inc-emojis">{emojis_html}</div>
              <div class="inc-feedback">
                <div class="bull-chip">🐂 {bulls} {bull_s}</div>
                <div class="cow-chip">🐄 {cows} {cow_s}</div>
              </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    # ── Interface de jogo ─────────────────────────────────────────────────────
        ...
        if len(cur) < INC_LEN:
            st.markdown(
                ...
                f"Escolha o animal para a <b>posição {len(cur)+1}</b>:</p>",
                unsafe_allow_html=True,
            )
            col_a, col_b = st.columns(2)
            with col_a:
                if st.button("🐄  Vaca", use_container_width=True):
                    st.session_state.inc_current.append("🐄"); st.rerun()
            with col_b:
                if st.button("🐂  Boi",  use_container_width=True):
                    st.session_state.inc_current.append("🐂"); st.rerun()
        ...
        for i, col in enumerate(cols):
            with col:
                ...
                st.markdown(
                    ...
                    f'box-shadow:0 3px 10px {color_hex(chosen)}88,inset 0 2px 4px rgba(255,255,255,.4);">'
                    f'</div></div>',
                    unsafe_allow_html=True)
        ...
        with c2:
            if st.button("🎲 Aleatório", use_container_width=True):
                st.session_state.current = (
                    ...
                    else random.sample(pool, st.session_state.code_len))
                st.rerun()
